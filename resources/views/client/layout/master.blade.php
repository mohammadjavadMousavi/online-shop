          ...
          <div id="top-links" class="nav pull-right flip">
            @auth()
              <form action="{{ route('client.logout') }}" method="post">
                ...
                <input type="submit" name="logout" value="خروج" class="btn btn-danger btn-sm">
          ...
          <div class="col-table-cell col-lg-3 col-md-3 col-sm-6 col-xs-12">
            <div id="cart">
              <button type="button" data-toggle="dropdown" data-loading-text="Loading..." class="heading dropdown-toggle">
                <span class="cart-icon pull-left flip">
                  ...
                  <span id="cart-total">
                   ...
                   {{--  @php
                      $item=collect(session()->get('cart'))->filter(function ($item)
                      {
                ...
                <li>
                  <table class="table">
                    <tbody id="cart-table-body">
                      ...
                      @foreach (\App\Models\Cart::getProduct() as $item)
                        
                      <tr id="cart-row-{{ $item['product']->id }}">
                        <td class="text-center"><a href="product.html"><img width="100" class="img-thumbnail" title="کفش راحتی مردانه" alt="کفش راحتی مردانه" src="{{ $item['product']->image_cart }}"></a></td>
                        <td class="text-left"><a href="product.html">{{ $item['product']->name }}</a></td>
                        <td class="text-right">x {{ $item['quantity'] }}</td>
                        <td class="text-right">{{ $item['product']->price_with_discount }} تومان</td>
                        <td class="text-center"><button class="btn btn-danger btn-xs remove" onclick="remove({{ $item['product']->id }});" title="حذف" onClick="" type="button"><i class="fa fa-times"></i></button></td>
                      </tr>
                    ...
                    <table class="table table-bordered">
                      <tbody>
                        <tr>
                          ...
                          <td class="text-right"><span id="total_amount_all">{{ \App\Models\Cart::totalAmount() }}</span> تومان</td>
                        </tr>
                        <tr>
                          <td class="text-right"><strong>قابل پرداخت</strong></td>
                          <td class="text-right"><span id="total_amount_payment">{{ \App\Models\Cart::totalAmount() }}</span> تومان</td>
                        </tr>
                      </tbody>
                    </table>
                    <p class="checkout"><a href="{{ route('client.cart.index') }}" class="btn btn-primary"><i class="fa fa-shopping-cart"></i> مشاهده سبد</a>&nbsp;&nbsp;&nbsp;<a href="{{ route('client.order.create') }}" class="btn btn-primary"><i class="fa fa-share"></i> تسویه حساب</a></p>
                  </div>
                </li>
              </ul>
            </div>
  ...
  function addToCart(productId){
    ...
    $.ajax({
      ...
      success: function(data){
        ...
        $('#total_amount_payment').text(data.cart.total_amount);
        $('#total_amount_all').text(data.cart.total_amount);


        $('#quantity-pro' + productId).text(data.cart[productId]['product']['price_with_discount'] * data.cart[productId]['quantity']);
        $('#total_amount_order').text(data.cart.total_amount);
        ...
        var productQty = data.cart[productId]['quantity'];
          ...
          $('#cart-table-body:last-child').append(
              ...
              + '<td class="text-center"><a href="product.html"><img width="100" class="img-thumbnail" title="کفش راحتی مردانه" alt="کفش راحتی مردانه" src=" ' + product.image_cart + ' "></a></td>' 
              ...
              + '<td class="text-center"><button class="btn btn-danger btn-xs remove" onclick="remove(' + product.id + ')" title="حذف" onClick="" type="button"><i class="fa fa-times"></i></button></td>' 
              +'</tr>'
            );
        }
