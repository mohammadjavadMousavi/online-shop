            ...
            <table class="table table-bordered table-hover">
              ...
              <tbody>
              	...
              	@foreach ($products as $product)
	                <tr>
	                  <td class="text-center"><a href="{{ route('client.product.show',$product) }}"><img width="100" src="{{ str_replace('public','/storage',$product->image) }}" alt="{{ $product->name }}" title="{{ $product->name }}" /></a></td>
	                  <td class="text-left"><a href="{{ route('client.product.show',$product) }}">{{ $product->name }}</a></td>
	                  <td class="text-left">{{ $product->brand->name }}</td>
	                  <td class="text-right"><div class="price"> {{ $product->price }} تومان </div></td>
	                  <td class="text-right"><button class="btn btn-primary" title="" data-toggle="tooltip" onClick="cart.add('48');" type="button" data-original-title="افزودن به سبد"><i class="fa fa-shopping-cart"></i></button>
	                 	...
	                 	<form action="{{ route('client.like.destroy',$product) }}" method="post">
	                 		@csrf
	                 		@method('DELETE')
